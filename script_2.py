...
def display_analytics(content_data):
    ...
    with col4:
        avg_rating = sum(c['rating'] for c in content_data['movies'] + content_data['tv_shows']) / \
                    len(content_data['movies'] + content_data['tv_shows'])
...
def filter_content(content_list, genre_filter, year_range, rating_filter):
    ...
    for content in content_list:
        if (genre_filter == "All" or content['genre'] == genre_filter) and \
           (year_range[0] <= content['year'] <= year_range[1]) and \
           (content['rating'] >= rating_filter):
...
# Save the advanced version
with open('streamflix_pro.py', 'w') as f:
    f.write(advanced_app_code)

print("Created streamflix_pro.py - Advanced streaming platform with enhanced features")

# Create setup instructions
setup_instructions = '''# StreamFlix Pro - Setup Instructions
...
print("Created README_SETUP.md with complete setup instructions")
print("\\n=== FILES CREATED ===")
print("1. streamflix_app.py - Basic streaming website")
print("2. streamflix_pro.py - Advanced streaming platform") 
print("3. content_database.json - Sample content database")
print("4. requirements.txt - Python dependencies")
print("5. README_SETUP.md - Setup and deployment guide")